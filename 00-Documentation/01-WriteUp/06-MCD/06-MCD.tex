...
\begin{table}[h]
  ...
  \begin{center}
    \begin{tabular}{|c|c|p{7cm}|}
      ...
      \textbf{Set method} & \textbf{Get method}  & \textbf{Comment}                                    \\
      \hline
      \texttt{setDQmode(DQmode)}   & \texttt{getDQmode(DQmode)}   & Set DQmode.                        \\
      \texttt{setLength(Length)}   & \texttt{getLength(Length)}   & Set length.                        \\
      \texttt{setStrength(Length)} & \texttt{getStrength(Length)} & Set strength (field gradient).     \\
      \texttt{setKDQ(Length)}      & \texttt{getKDQ(Length)}      & Set kDQ, quadrupole $k$ parameter. \\
      \texttt{setTransferMatrix()} &                              & Set transfer matrix.               \\
      \hline
    \end{tabular}
  \end{center}
...
\begin{table}[h]
  \caption{
    Utilities provided by the \texttt{SectorDipole} derived
    class. 
  }
  \label{Tab:SDpl:Methods}
  \begin{center}
