 ...
 getParabolic : Generate random number distributed as an inverted parabola
  ...
  Get methods:
      ...
      getRandomSeed: Returns random seed
        ...
        getLhARAbeam: Get instance of LhARAbeam class
        ...
        if not isinstance(ri, complex):
            if ri >= -p1:
                if ri <= p1:
                    ...
                    p = ri
    if isol != 1:
        raise Exception("LhARASimu.getParabolic; p multiply defined")
...
#--------  "Built-in methods":
    def __new__(cls, NEvt=5, _filename=None, \
        ...
        if cls.__instance is None:
            print('LhARASimu.__new__: creating the Simulation object')
            print('-----------------')
            ...
            # Create LhARAbeam instance:
            cls.__LhARAbm = LhARAbeam.LhARAbeam(_filename)
...
#.. Methods believed to be self documenting(!)
    ...
    @classmethod
    def getLhARAbeam(cls):
        return cls.__LhARAbm
...
#--------  Simulation run methods
    def RunSim(self):
        print()
        print('LhARASimu.RunSim: simulation begins')
        print('----------------')
        ...
        nEvt = self.getLhARAbeam().trackLhARA(self.getNEvt(), ParticleFILE)
