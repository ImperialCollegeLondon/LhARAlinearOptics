...
================
  ...
  CEO class for linear optics simulation of LION beam lines.
 ...
 getParabolic : Generate random number distributed as an inverted parabola
  ...
  Get methods:
      ...
      getRandomSeed: Returns random seed
        ...
        getLIONbeam: Get instance of LIONbeam class
        ...
        if not isinstance(ri, complex):
            if ri >= -p1:
                if ri <= p1:
                    ...
                    p = ri
    if isol != 1:
        raise Exception("LIONSimu.getParabolic; p multiply defined")
...
#--------  "Built-in methods":
    def __new__(cls, NEvt=5, _filename=None, \
        ...
        if cls.__instance is None:
            print('LIONSimu.__new__: creating the Simulation object')
            print('-----------------')
            ...
            # Create LIONbeam instance:
            cls.__LIONbm = LIONbeam.LIONbeam(_filename)
...
#--------  Simulation run methods
    def RunSim(self):
        print()
        print('LIONSimu.RunSim: simulation begins')
        print('----------------')
        ...
        nEvt = self.getLIONbeam().trackLION(self.getNEvt(), ParticleFILE)
