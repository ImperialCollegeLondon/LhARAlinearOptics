...
class BeamLine(object):
    ...
    def __new__(cls, _BeamLineSpecificationCVSfile=None):
        if cls.getinstance() == None:
            if cls.getDebug():
                print(" BeamLine.__new__: ", "creating the BeamLine object")
            ...
            if _BeamLineSpecificationCVSfile == None:
                raise Exception(" BeamLine.__new__: no parameter file given.")

            if not os.path.exists(_BeamLineSpecificationCVSfile):
                print(
                    " BeamLine.__New__: _BeamLineSpecificationCVSfile:",
                    _BeamLineSpecificationCVSfile,
                )
                raise Exception(" BeamLine.__new__: parameter file does not exist.")

            cls._BeamLineSpecificationCVSfile = _BeamLineSpecificationCVSfile
            cls._BeamLineParamPandas = BeamLine.csv2pandas(
                _BeamLineSpecificationCVSfile
            )
            if not isinstance(cls._BeamLineParamPandas, pnds.DataFrame):
                raise Exception(" BeamLine.__new__: pandas data frame invalid.")

            cls._Element = []

            if cls.getDebug():
                print(
                    "     ----> Parameter file: ", cls.getBeamLineSpecificationCVSfile()
                )
                print(
                    "     ----> Dump of pandas paramter list: \n",
    ...
    def __str__(self):
        ...
        print("     ----> Beam line is self consistent = ", self.checkConsistency())
    ...
    def setSrcTrcSpc(cls, SrcTrcSpc=np.array([])):
        ...
        if not isinstance(SrcTrcSpc, np.ndarray):
            raise badTraceSpaceVector(" BeamLine.setSrcTrcSpc:", SrcTrcSpc)
    ...
    def addFacility(cls):
        ...
        rStrt = np.array([0.0, 0.0, 0.0])
        vStrt = np.array([0.0, 0.0, 0.0])
        drStrt = np.array([0.0, 0.0, 0.0])
        dvStrt = np.array([0.0, 0.0, 0.0])

        p0 = mth.sqrt((protonMASS + K0) ** 2 - protonMASS**2)

        FacilityBLE = BLE.Facility(Name, rStrt, vStrt, drStrt, dvStrt, p0, VCMVr)
        cls._Element.append(FacilityBLE)

        if cls.getDebug():
            print("             <----", Name, "facility initialise.")
    ...
    def addSource(cls):
        ...
        SourceBLE = BLE.Source(Name, rStrt, vStrt, drStrt, dvStrt, SrcMode, SrcParam)
        ...
        if cls.getDebug():
            print("                 ---->", Name, "beam line element created.")
            print("                     ----> reference particle:")
    ...
    def parseFacility(cls):
        ...
        Name = str(
            pndsFacility[
                (pndsFacility["Type"] == "Name") & (pndsFacility["Parameter"] == "Name")
            ].iloc[0]["Value"]
        )
        K0 = float(
            pndsFacility[
        ...
        if cls.getDebug():
            print(
                "                     ----> Name:",
                Name,
                "; reference particle kinetic energy:",
                K0,
                "MeV",
            )
            print(
    ...
    def parseSource(cls):
        ...
        if SrcMode == 0:  # .. Laser driven:
            Emin = float(pndsSource[pndsSource["Parameter"] == "Emin"]["Value"].iloc[0])
            Emax = float(pndsSource[pndsSource["Parameter"] == "Emax"]["Value"].iloc[0])
            nPnts = int(pndsSource[pndsSource["Parameter"] == "nPnts"]["Value"].iloc[0])
            MinCTheta = float(
        ...
        SigmaX = float(pndsSource[pndsSource["Parameter"] == "SigmaX"]["Value"].iloc[0])
        SigmaY = float(pndsSource[pndsSource["Parameter"] == "SigmaY"]["Value"].iloc[0])
        ...
        Name = (
            BLE.BeamLineElement.getinstances()[0].getName()
            + ":"
    ...
    def addBeamline(cls):
        ...
        p0 = mth.sqrt(np.dot(iRefPrtcl.getPrIn()[0][:3], iRefPrtcl.getPrIn()[0][:3]))
        ...
        for iLine in pndsBeamline.itertuples():
            ...
            if iLine.Element == "Drift":
                ...
                if cls.getDebug():
                    print("             ----> Add", Name)
                iBLE = BLE.Aperture(Name, rStrt, vStrt, drStrt, dvStrt, Param)
                cls._Element.append(iBLE)
                s += 0.0
                refPrtcl = Prtcl.ReferenceParticle.getinstance()
                refPrtclSet = refPrtcl.setReferenceParticleAtDrift(iBLE)
                ...
                if iLine.Type == "Length, layers and turns":
                    ...
                    raise badParameter(
                        " BeamLine.addbeam: Solenoid", " Type=", iLine.Type, " invalid."
                    )
                if cls.getDebug():
                    print("             ----> Add", Name)
                ...
                if iLine.Parameter == "Gradient":
                    ...
                    CylCvtyPhs = CylCvtyPhs * mth.pi / 180.0
                if iLnCvty < nLnsCvty:
                    iLnCvty += 1
                    NewElement = False
                    continue
    ...
    def checkConsistency(self):
        ConsChk = False
        s = 0.0
        if self._Element[0].getrStrt()[2] != 0.0:
            return
        for iBLE in BLE.BeamLineElement.getinstances():
    ...
    def trackBeam(cls, NEvts=0, ParticleFILE=None):
        ...
        for iEvt in range(1, NEvts + 1):
            ...
            if isinstance(cls.getSrcTrcSpc(), np.ndarray):
                ...
                SrcTrcSpc = cls.getElement()[1].getParticleFromSource()
            Success = PrtclInst.recordParticle(Name, 0.0, 0.0, SrcTrcSpc)
            if cls.getDebug():
                print("     ----> Event", iEvt)
                with np.printoptions(linewidth=500, precision=7, suppress=True):
            ...
            for iBLE in BLE.BeamLineElement.getinstances():
                iLoc += 1
                if isinstance(iBLE, BLE.Source) or isinstance(iBLE, BLE.Facility):
                    continue
                if cls.getDebug():
                    print("         ---->", iBLE.getName())
                ...
                if cls.getDebug():
                    with np.printoptions(linewidth=500, precision=7, suppress=True):
                        print("             ----> Updated trace space   :", TrcSpc)
