...
class BeamlinePlotter(object):
    ...
    def Tracer(
        ...
        LhARA_Bl = BL.BeamLine(HOMEPATH + "/11-Parameters/" + facility)
            ...
            while not EndOfFile:
                ...
                if iPrtcl.getz()[-1] > maxz:  # the ones that get to the end of beamline
                    if mini < iPrtcl.getTraceSpace()[0][5] + RefE < maxi:
                        ...
                        x_lab = []
                        y_lab = []
                        z_lab = []#not sure about this one

                        print(len(particle.getTraceSpace())
                              )
                        ...
                        print(len(particle.getLabPhaseSpace()))
                        print(particle._LabPhsSpc)
                        
                        for i in range(len(particle.getLabPhaseSpace())):                            
                            x_lab.append(particle.getLabPhaseSpace()[i][0])                            
                            y_lab.append(particle.getLabPhaseSpace()[i][2])
                            z_lab.append(particle.getLabPhaseSpace()[i][4])
    ...
    def plt_apt(
        filename, HOMEPATH=os.getenv("HOMEPATH")
    ):  # automatize the position of the apertures in the beamline
        params = pd.read_csv(HOMEPATH + "/11-Parameters/" + filename)
    ...
    def plt_quad(filename, HOMEPATH=os.getenv("HOMEPATH")):
        filedir = os.path.join(HOMEPATH, "11-Parameters/" + filename)
        ...
        j = find_rows(
            params, "Dquad", use_include=True, include="Length"
        )  # esto incluye la fila con strength
        for i in range(len(j)):  # formerly j
            ...
            quad_coords = [
                (quad_posi - quad_lnth, ori * 6e-3),
                (quad_posi - (quad_lnth), ori * 5e-3),
                (quad_posi, ori * 5e-3),
                (quad_posi, ori * 6e-3),
                (quad_posi - quad_lnth, ori * 6e-3),
            ]
    ...
    def plt_prtclnumberAFR(
        ...
        for i in range(len(z)):  # number of co-ordinates to read per particle
            ...
            # ParticleFILE = Prtcl.Particle.openParticleFile("99-Scratch", "DRACOsimuLsrDrvn.dat")
            ParticleFILE = Prtcl.Particle.openParticleFile(
                HOMEPATH + "/99-Scratch", "DRACOsimuLsrDrvn.dat"
            )
            ...
            partic_left = (
                count / Nevts
            )  # add particular ase in which we go through all the particles

            print(partic_left)

        axs.plot(
            z[0:10], n_den[0:10], label="Transmission efficiency: %.3f" % partic_left
        )
        axs.grid(True)
...
def plt_colim(z):
    ...
    col_x = np.array(
        [-3 * col_rad, -col_rad, None, col_rad, 3 * col_rad]
    )  # 6E-3 set arbitrarily
