...
#-------"Get" Methods-------------------- 
    ...
    def get_zlist(HOMEPATH = os.getenv('HOME'), filename = None):
    ...
    def get_elementlist(HOMEPATH = os.getenv('HOME'), filename = None):
...
#--------Beamline plotting Methods---------------    
    ...
    def Tracer(cls, mini, maxi, maxz = 2.136, HOMEPATH = os.getenv('HOME'), filename = None, label = None, NEvts = 5, alpha = 0.5, colour = 'r'):
        ...
        facilities = ['LhARA', 'LION', 'DRACO']
    
        for file_name in file_names:
            for facility in facilities:
                if facility in file_name:
                    print(f"Facility: {facility}")
                    cls.__Facility = facility
                if facility not in file_name:
        ...
        try:
                ...
                while not EndOfFile:
                        ...
                        iPrtcl = Prtcl.Particle.getParticleInstances()[0] #used to be 0
            
                        if iPrtcl.getz()[-1] > maxz: #the ones that get to the end of beamline
            
                                    if mini < iPrtcl.getPhaseSpace()[0][5] < maxi:
                                            ...
                                            for i in range(len(particle.getPhaseSpace())):
                                                  x.append(particle.getPhaseSpace()[i][0])
                                                  y.append(particle.getPhaseSpace()[i][2])
    ...
    def plt_apt(cls): #automatize the position of the apertures in the beamline
            ...
            z        = cls.get_zlist(HOMEPATH  = HOMEPATH, filename = filename)
            elements = cls.get_elementlist(HOMEPATH  = HOMEPATH, filename = filename)
    ...
    def plt_solenoid(cls):
            ...
            for i in range(len(j)):
                    ...
                    sole_posi = z[index[i]]
                    sole_rad  = params.iloc[j[i]-1][5]
                    sole_lnth = params.iloc[j[i]][5]
    ...
    def plt_prtclnumberAFR(cls, fig, axs ,Nevts = 5000, data_file = None, parameter_file = None):
            ...
            z = cls.get_zlist(HOMEPATH = HOMEPATH, filename = parameter_file)        
