...
try:
    FQuad = BLE.FocusQuadrupole("NoStrength", \
...
R      = np.array([0.3, 0.1, -0.4, -0.2, 0.1, 0.5])
...
RprimeTest = np.array([ 0.218783514, -1.637577772, -0.555844114, \
                        -3.07986627, 14.05601118, 0.5 ])
with np.printoptions(linewidth=500,precision=5,suppress=True): \
...
with np.printoptions(linewidth=500,precision=5,suppress=True): \
     print("     ----> Transported phase-space vector:", Rprime)
RprimeTest = np.array([ 0.418783514, -1.637577772, -0.455844114, \
                        -3.079866272, 14.05601118, 0.5 ])
with np.printoptions(linewidth=500,precision=5,suppress=True): \
     print("     ----> Pre-calculated          Rprime:", RprimeTest)
Diff       = np.subtract(Rprime, RprimeTest)
Norm       = np.linalg.norm(Diff)
