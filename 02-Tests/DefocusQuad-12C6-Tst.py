...
try:
    DfQuad = BLE.DefocusQuadrupole("NoStrength", \
...
RprimeTest = np.array([0.411337669, 2.243191898, -0.297695477, \
                       2.136813179, 14.05601118, 0.5] )
with np.printoptions(linewidth=500,precision=5,suppress=True): \
...
with np.printoptions(linewidth=500,precision=5,suppress=True): \
     print("     ----> Transported phase-space vector:", Rprime)
RprimeTest = np.array([ 0.611337669, 2.243191898, -0.197695477, \
                        2.136813179, 14.05601118, 0.5 ])
with np.printoptions(linewidth=500,precision=5,suppress=True): \
     print("     ----> Pre-calculated          Rprime:", RprimeTest)
Diff       = np.subtract(Rprime, RprimeTest)
